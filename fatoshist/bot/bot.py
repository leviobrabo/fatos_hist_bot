...
class Bot:
    ...
    def register_handlers(self):
        ...
        def got_payment(message):
            try:
                ...
                caption_success = (
                    "Doação bem-sucedida! "
                    "Você contribuiu para o projeto História, "
                    "ajudando a manter este projeto funcionando."
                )
    ...
    def handle_donate(self, call):
        ...
        btn_50 = types.InlineKeyboardButton(
            "⭐️ 50 Estrelas", callback_data="50_estrelas"
        )
        btn_100 = types.InlineKeyboardButton(
            "⭐️ 100 Estrelas", callback_data="100_estrelas"
        )
        btn_200 = types.InlineKeyboardButton(
            "⭐️ 200 Estrelas", callback_data="200_estrelas"
        )
        btn_500 = types.InlineKeyboardButton(
            "⭐️ 500 Estrelas", callback_data="500_estrelas"
        )
        btn_1000 = types.InlineKeyboardButton(
            "⭐️ 1000 Estrelas", callback_data="1000_estrelas"
        )
    ...
    def handle_config(self, call):
        ...
        if user_info:
            ...
            msg_text += (
                f"<b>Recebe mensagem no chat privado:</b> "
                f'{"Sim" if user_info["msg_private"] == "true" else "Não"}\n'
            )
            ...
            if user_info["questions"] > 0:
                ...
                msg_text += (
                    f"<b>Porcentagem de acerto por questões:</b> "
                    f"<code>{percentage:.2f}%</code>\n"
                )
            else:
