...
def schedule_tasks(bot: TeleBot):
    try:
        ...
        schedule.every().day.at('21:08').do(lambda: hist_chat_job(bot))

        # Envio eventos histórico no users
        schedule.every().day.at('21:15').do(lambda: hist_user_job(bot))

        # Envio eventos histórico no channel
        schedule.every().day.at('21:08').do(lambda: hist_channel_events(bot))
        ...
        schedule.every().day.at('21:18').do(lambda: get_holidays_br_and_world_of_the_day(bot))

        # Envio de Fotos históricas no grupo
        schedule.every().day.at('21:22').do(lambda: hist_image_chat_job(bot))

        # Envio de Fotos históricas no canal
        schedule.every().day.at('21:26').do(lambda: hist_channel_imgs(bot))
        ...
        schedule.every().day.at('21:33').do(lambda: hist_channel_curiosity(bot))

        # Envio de frases no canal
        schedule.every().day.at('21:39').do(lambda: hist_channel_frase(bot))

        # Envio dos presidentes no canal
        schedule.every().day.at('23:00').do(lambda: enviar_foto_presidente(bot))

        # Envio da historia diaria
        schedule.every().day.at('21:45').do(lambda: hist_channel_history(bot))
